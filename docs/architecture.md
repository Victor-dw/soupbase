...
`src/server/host.ts` 定义 `host-1.5-facts-first`，使用 Choice：yes / no / irrelevant / uncertain。
...
`src/server/guess.ts` 定义 `guess-1.2-context`。每个必需关键点使用 Choice 判断成立/缺失/矛盾，另检查整体解释。结合公开汤面理解简短还原，不要求重复已知事实，也不从汤底替玩家补上缺失的核心机制。
